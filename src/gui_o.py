...
class EmailSenderGUI:
    def __init__(self, root):
        ...
        # Excel upload
        upload_btn = Button(root, text="Upload Excel", command=self.upload_excel)
        ...
        self.cc_entry = Entry(root, width=50)
        self.cc_entry.grid(row=2, column=1, padx=10, pady=10)

        # Message text box
        Label(root, text="Message:").grid(row=3, column=0, sticky='ne')
        self.message_text = Text(root, width=50, height=10)
        self.message_text.grid(row=3, column=1, padx=10, pady=10)

        # Signature text box
        Label(root, text="Signature:").grid(row=4, column=0, sticky='ne')
        self.signature_text = Text(root, width=50, height=5)
        self.signature_text.grid(row=4, column=1, padx=10, pady=10)

        # Attachment upload
        self.attachment_label = Label(root, text="No attachment")
        self.attachment_label.grid(row=5, column=0, padx=10, pady=10, sticky='w')
        upload_attachment_btn = Button(root, text="Upload Attachment", command=self.upload_attachment)
        upload_attachment_btn.grid(row=5, column=1, padx=10, pady=10, sticky='w')

        # Start sending button
        start_btn = Button(root, text="Start Sending Emails", command=self.send_emails)
        start_btn.grid(row=6, column=1, padx=10, pady=10)

        # Credentials list
        Label(root, text="Credentials:").grid(row=7, column=0, padx=10, pady=10, sticky='nw')
        self.credentials_listbox = Listbox(root, selectmode=MULTIPLE)
        self.credentials_listbox.grid(row=7, column=1, padx=10, pady=10, sticky='w')

        # Add, Edit, Delete buttons for credentials
        add_cred_btn = Button(root, text="Add Credential", command=self.add_credential)
        add_cred_btn.grid(row=8, column=1, padx=10, pady=10, sticky='w')
        edit_cred_btn = Button(root, text="Edit Credential", command=self.edit_credential)
        edit_cred_btn.grid(row=8, column=1, padx=10, pady=10, sticky='e')
        delete_cred_btn = Button(root, text="Delete Selected Credential(s)", command=self.delete_selected_credentials)
        ...
        save_template_btn = Button(self.root, text="Save Template", command=self.save_template)
        save_template_btn.grid(row=10, column=1, padx=10, pady=10, sticky='w')

        load_template_btn = Button(self.root, text="Load Template", command=self.load_template)
        load_template_btn.grid(row=10, column=1, padx=10, pady=10, sticky='e')
        
        # Create a listbox for template selection
        self.template_listbox = Listbox(self.root)
        self.template_listbox.grid(row=11, column=1, padx=10, pady=10)
        self.update_template_list()

        # Edit and delete buttons for templates
        edit_template_btn = Button(self.root, text="Edit Template", command=self.edit_template)
        edit_template_btn.grid(row=12, column=1, padx=10, pady=10, sticky='w')
        
        delete_template_btn = Button(self.root, text="Delete Template", command=self.delete_template)
        delete_template_btn.grid(row=12, column=1, padx=10, pady=10, sticky='e')
